...
il quanto più vago possibile rispetto alla rappresentazione interna:
...
utilizzare qualsiasi tipo numerico per rappresentare le coordinate, senza dover riscrivere 
il codice per ogni tipo. Inoltre, grazie all'uso di using, è possibile utilizzare le operazioni 
aritmetiche sui numeri senza dover specificare il tipo numerico ogni volta.
Inoltre, sfruttando la conversione implicita, è possibile convertire un Poin2D in una tupla
senza bisogno di cast espliciti:
\begin{lstlisting}[language=Scala]
...
given [N](using Numeric[N]): Conversion[Point2D[N], (N, N)] with
  def apply(p: Point2D[N]): (N, N) =
    (p.x, p.y)
\end{lstlisting}
Questo ha permesso poi di poter creare, a seconda del gioco, il proprio tipo di Point2D. Inoltre,
grazie ai tipi opachi di Scala, non è necessario esporre il tipo numerico, ad esempio nel caso di
MazePosition:
\begin{lstlisting}[language=Scala]
object MazePosition:
...
\end{lstlisting}
Grazie poi agli extension methods, è possibile decidere quali funzionalità di Point2D esporre.
...
se si volesse implementare l'idea di poter perdere un oggetto dopo averlo raccolto.
\begin{lstlisting}[language=Scala]
trait Pocket:
...
Il primo aspetto è stato quello di costruire un costruttore di Maze che potesse assicurare la
corretta generazione del labirinto, soddisfacendo dei requisiti specifici da me imposti.
Infatti, ho reputato che un labirinto potesse essere definito tale se:
\begin{itemize}
...
prendere esempio da il progetto di Jahrim Gabriele Cesario\cite{pps22chess}, che ha implementato un generatore di
scacchiere. Il mio codice adattato per la DSL (Domain Specific Langauge) del labirinto è il seguente:
\begin{lstlisting}[language=Scala]
object DSL:
...
riconoscere facilmente in quale direzione potesse muoversi.
...
private def computeSolution(): Option[List[MazePosition]] =
  ...
  ...
\end{lstlisting}
Inoltre, il costrutto Scala di "lazy" permette di calcolare la soluzione solamente una volta, evitando
...
Nello specifico, si tratta di un North-East Tree, e ha questo punto di entrata:
\begin{lstlisting}[language=Prolog]
...
l'algoritmo è deterministico e non richiede un generatore di numeri casuali interno, non disponibile
nella versione di Prolog utilizzata; come valore aggiunto maggiore è la randomicità dei numeri 
binari, maggiore sarà la difficoltà del labirinto generato.  
