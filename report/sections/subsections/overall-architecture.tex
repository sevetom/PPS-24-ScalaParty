...
plancia. Questa struttura consentirà di espandere più facilmente l'applicazione in futuro e 
aumenta la manutenibilità del codice, oltre a permettere una migliore suddivisione dei 
compiti.

Per questo, l'architettura si divide nei seguenti componenti principali:
\begin{itemize}
    \item \textbf{Controller}: è il componente principale del gioco. Gestisce le
    regole e le interazioni tra i vari componenti. Sfrutta le funzionalità del gioco
    per assegnare il turno ai giocatori e gestire le condizioni di vittoria.
    \item \textbf{View}: è la parte dell'applicazione che si occupa di mostrare
    l'interfaccia utente e di gestire le interazioni con il giocatore.
    \item \textbf{PartyGame}: racchiude la logica di gioco generale
    come il lancio dei dadi, il conteggio dei punti e la gestione dei componenti del gioco.
    ...
    che simula le azioni di un giocatore umano cercando di comportarsi nella maniera
    più intelligente possibile.
    \item \textbf{Minigame}: viene creato all'inizio di un minigioco la cui logica viene
    scelta tra quelle disponibili. Gestisce le regole specifiche del
...
nella maggior parte dei casi, sia il \texttt{Controller} a iniziare le interazioni
...
\begin{itemize}
    ...
    una nuova partita e il \texttt{Controller} segnala al \texttt{PartyGame} di 
    ...
    Il \texttt{Controller} chiede al \texttt{PartyGame} le informazioni necessarie
    ...
    plancia in base al risultato del lancio dei dadi. Il \texttt{Controller} chiede
    ...
    disponibili. 
    \item \textbf{Minigame}: è il momento in cui viene avviato un nuovo minigioco.
\end{itemize}
