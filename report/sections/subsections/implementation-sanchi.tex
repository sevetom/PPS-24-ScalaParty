...
un rung e tutte le posizioni delle celle espresse come tupla di coordinate e contenuto della cella. Successivamente, tramite le
regole, vengono definite le azioni con cui viene scelto il percorso migliore. L'algoritmo valuta innanzitutto quale sia l'oggetto
collezionabile più vicino all'avversario tramite il metodo di manhattan e successivamente calcola il percorso migliore per
raggiungerlo. Ciò avviene tramite un algorimto di ricerca bfs (breadth-first search). Durante la ricerca, l'alogritmo tiene conto
solo delle celle valite (ovvero quelle definite nei fatti) sapendo di poter costruire un percorso solo con esse e passando su celle
tra loro adiacenti. Infine l'algoritmo restituisce la lista di celle da percorrere per raggiungere l'oggetto collezionabile.
...
La modellazione del gioco è stata efefttuata da tutti e tre i membri del gruppo, ma in particolare ho lavorato sulla gestione
...
L'intera funzione è scritta in stile funzionale, senza modificare direttamente lo stato del gioco; quindia d ogni passo:
